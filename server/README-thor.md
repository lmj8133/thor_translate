...
**最省事的方式：看 Termux 畫面**——啟動 proxy 後會直接印出一個框起來的 `READY`／`NOT READY` 橫幅，看到 READY 就能開遊戲。

想看詳細狀態時，Thor 的瀏覽器開 **`http://127.0.0.1:8000/status`**。頁面**不會自動刷新**（開著也不耗電），要更新就手動下拉重新整理。最上方那條綠色橫幅就是就緒狀態：
