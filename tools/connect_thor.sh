...
# -> ping-sweep of the Windows host's subnets (via ipconfig.exe).
#
...
ADB="${ADB:-$HOME/thor-work/platform-tools/adb}"
...
sweep_subnets() {
    # Windows-side IPv4 subnets (WSL's own NAT subnet is excluded: the Thor
    # can never be there). ipconfig output may be non-UTF8 - filter roughly.
    local wsl_gw
