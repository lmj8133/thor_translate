...
>
> **2026-08-03 擴充 #5:Thor 單機模式(Termux)**。動機:使用者問「server 能不能開在 iPhone」;查證結論 iOS 無法可靠背景長駐(iSH+location hack 屬脆弱 hack),正解 = 代理搬進 Thor 的 Termux,PT 連 127.0.0.1、翻譯走雲端,Mac 退為在家時的可選兜底,iPhone 的角色 = 熱點。配套:代理依賴全面純 Python 化(FastAPI→Starlette、C++ opencc→opencc-python-reimplemented,import 相容、s2twp/tw2sp 測試全過)→ Termux 免編譯 pip 直裝;部署資產 server/thor/(setup-termux.sh / run-proxy.sh / boot-start.sh / healthcheck.py)與指南 server/README-thor.md;phantom process killer 已由 adb 關閉。**2026-08-03 實機部署完成並驗證**:Termux+Termux:Boot 由 adb 安裝(GitHub v0.118.3),依賴 headless 裝妥(教訓:Termux 禁止 pip 自我升級,腳本已修),代理於裝置上啟動、三雲端模型煙霧測試全 OK(Gemma 首次冷啟 36s,故 uvicorn 開埠比 log 晚),裝置上 loopback 端到端實測「ポケモンはかせ」→「寶可夢博士」正確。測試 36 綠。
>
